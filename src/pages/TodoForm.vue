        ...
        <el-scrollbar max-height="710px" >
          <el-main>
            <el-table :data="filterTodoList" border empty-text="暂无内容">
              ...
              <el-table-column prop="startTime" label="创建时间" width="400" />
              <el-table-column prop="endTime" label="完成时间" width="400" />
              <el-table-column label="操作" width="300">
                <template v-slot="scope">
                  <div v-show="!scope.row.status">
                    ...
                    <el-button round @click="todoDelete(scope.row.tid)">删除</el-button>
                  </div>
                  <div v-show="scope.row.status">
...
<script setup>
  ...
  import { getTask, updataTask, deleteTask } from '../api/task.js'
  ...
  const filterTodoList = computed(() =>
    todoList.filter( (data) =>
        (!date.value || data.startTime.split(' ')[0] === date.value)
        &&
        (!search.value || data.content.includes(search.value))
    )
  )
  ...
  function todoComplete(rowData) {
    ...
    updataTask(rowData.tid, token, data)
    .then(
      res => {
        if (res.code === 200) {
          todoListStore.handleCompelete(rowData.tid, data.endTime)
      ...
      .then(
        res => {
          if (res.code === 200) {
            ...
            todoListStore.handleEdit(rowData.tid, value)
          } else {
            ElNotification({
              title: 'Error',
              message: '修改失败！',
